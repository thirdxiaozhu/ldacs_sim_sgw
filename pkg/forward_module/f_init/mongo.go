...
func (m *mongo) Initialization() error {
	...
	if f_global.GVA_CONFIG.Mongo.IsZap {
		opts = internal.Mongo.GetClientOptions()
	}
	ctx := context.Background()
	client, err := qmgo.Open(ctx, &qmgo.Config{
		Uri:              f_global.GVA_CONFIG.Mongo.Uri(),
		Coll:             f_global.GVA_CONFIG.Mongo.Coll,
		Database:         f_global.GVA_CONFIG.Mongo.Database,
		MinPoolSize:      &f_global.GVA_CONFIG.Mongo.MinPoolSize,
		MaxPoolSize:      &f_global.GVA_CONFIG.Mongo.MaxPoolSize,
		SocketTimeoutMS:  &f_global.GVA_CONFIG.Mongo.SocketTimeoutMs,
		ConnectTimeoutMS: &f_global.GVA_CONFIG.Mongo.ConnectTimeoutMs,
		Auth: &qmgo.Credential{
			Username: f_global.GVA_CONFIG.Mongo.Username,
			Password: f_global.GVA_CONFIG.Mongo.Password,
		},
	}, opts...)
	if err != nil {
		return errors.Wrap(err, "链接mongodb数据库失败!")
	}
	f_global.GVA_MONGO = client
	err = m.Indexes(ctx)
	if err != nil {
		return err
	}
...
func (m *mongo) CreateIndexes(ctx context.Context, name string, indexes [][]string) error {
	collection, err := f_global.GVA_MONGO.Database.Collection(name).CloneCollection()
	if err != nil {
	...
	for k1, v1 := range indexMap1 {
		...
		if len(fmt.Sprintf("%s.%s.$%s", collection.Name(), name, v1)) > 127 {
			err = f_global.GVA_MONGO.Database.Collection(name).CreateOneIndex(ctx, options.IndexModel{
				Key:          v1,
