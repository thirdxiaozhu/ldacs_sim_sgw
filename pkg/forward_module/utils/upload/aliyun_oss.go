...
func (*AliyunOSS) UploadFile(file *multipart.FileHeader) (string, string, error) {
	...
	yunFileTmpPath := f_global.GVA_CONFIG.AliyunOSS.BasePath + "/" + "uploads" + "/" + time.Now().Format("2006-01-02") + "/" + file.Filename
	...
	return f_global.GVA_CONFIG.AliyunOSS.BucketUrl + "/" + yunFileTmpPath, yunFileTmpPath, nil
}
...
func NewBucket() (*oss.Bucket, error) {
	// 创建OSSClient实例。
	client, err := oss.New(f_global.GVA_CONFIG.AliyunOSS.Endpoint, f_global.GVA_CONFIG.AliyunOSS.AccessKeyId, f_global.GVA_CONFIG.AliyunOSS.AccessKeySecret)
	if err != nil {
		return nil, err
	}

	// 获取存储空间。
	bucket, err := client.Bucket(f_global.GVA_CONFIG.AliyunOSS.BucketName)
	if err != nil {
		return nil, err
	}
