...
import (
	...
	"ldacs_sim_sgw/pkg/forward_module/utils/ast"
	"path/filepath"
	"strconv"
	"strings"
	"time"
...
func (autoCodeHistoryService *AutoCodeHistoryService) CreateAutoCodeHistory(meta, structName, structCNName, autoCodePath string, injectionMeta string, tableName string, apiIds string, Package string, BusinessDB string) error {
	return forward_global.GVA_DB.Create(&system.SysAutoCodeHistory{
		Package:       Package,
		RequestMeta:   meta,
...
func (autoCodeHistoryService *AutoCodeHistoryService) First(info *request.GetById) (string, error) {
	var meta string
	return meta, forward_global.GVA_DB.Model(system.SysAutoCodeHistory{}).Select("request_meta").Where("id = ?", info.Uint()).First(&meta).Error
}
...
func (autoCodeHistoryService *AutoCodeHistoryService) Repeat(businessDB, structName, Package string) bool {
	var count int64
	forward_global.GVA_DB.Model(&system.SysAutoCodeHistory{}).Where("business_db = ? and struct_name = ? and package = ? and flag = 0", businessDB, structName, Package).Count(&count)
	return count > 0
}
...
func (autoCodeHistoryService *AutoCodeHistoryService) RollBack(info *systemReq.RollBack) error {
	...
	if err := forward_global.GVA_DB.Where("id = ?", info.ID).First(&md).Error; err != nil {
		return err
	}
	// 清除API表
	...
	if err != nil {
		forward_global.GVA_LOG.Error("ClearTag DeleteApiByIds:", zap.Error(err))
	}
	// 删除表
	if info.DeleteTable {
		if err = AutoCodeServiceApp.DropTable(md.BusinessDB, md.TableName); err != nil {
			forward_global.GVA_LOG.Error("ClearTag DropTable:", zap.Error(err))
		}
	}
	// 删除文件
	...
	for _, path := range strings.Split(md.AutoCodePath, ";") {
		...
		// 迁移
		nPath := filepath.Join(forward_global.GVA_CONFIG.AutoCode.Root,
		...
		if err != nil {
			forward_global.GVA_LOG.Error("file move err ", zap.Error(err))
		}
	...
	md.Flag = 1
	return forward_global.GVA_DB.Save(&md).Error
}
...
func (autoCodeHistoryService *AutoCodeHistoryService) Delete(info *request.GetById) error {
	return forward_global.GVA_DB.Where("id = ?", info.Uint()).Delete(&system.SysAutoCodeHistory{}).Error
}
...
func (autoCodeHistoryService *AutoCodeHistoryService) GetList(info request.PageInfo) (list []response.AutoCodeHistory, total int64, err error) {
	...
	db := forward_global.GVA_DB.Model(&system.SysAutoCodeHistory{})
